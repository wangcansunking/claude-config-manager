...
export async function handleListPlugins(managers: QueryToolManagers) {
...
export async function handleListMcpServers(managers: QueryToolManagers) {
...
export async function handleListSkills(managers: QueryToolManagers) {
...
export async function handleListCommands(managers: QueryToolManagers) {
...
export async function handleGetConfig(
  managers: QueryToolManagers,
  args: { section?: string },
) {
  const settings = await managers.configManager.getSettings();
  const result =
    args.section !== undefined && args.section !== ''
...
export async function handleGetComponentDetail(
  managers: QueryToolManagers,
  args: { type: 'plugin' | 'mcp' | 'skill'; name: string },
) {
...
export function registerQueryTools(server: McpServer, managers: QueryToolManagers): void {
  server.tool(
    'ccm_list_plugins',
    'List all installed Claude plugins',
    async () => handleListPlugins(managers),
  );

  server.tool(
    'ccm_list_mcp_servers',
    'List all configured MCP servers',
    async () => handleListMcpServers(managers),
  );

  server.tool(
    'ccm_list_skills',
    'List all skills from installed plugins',
    async () => handleListSkills(managers),
  );

  server.tool(
    'ccm_list_commands',
    'List all user-defined commands',
    async () => handleListCommands(managers),
  );

  server.tool(
    'ccm_get_config',
    'Get the current Claude configuration settings',
    { section: z.string().optional().describe('Optional config section to retrieve') },
    async (args) => handleGetConfig(managers, args),
  );

  server.tool(
    'ccm_get_component_detail',
    'Get detailed information about a specific plugin, MCP server, or skill',
    {
      type: z

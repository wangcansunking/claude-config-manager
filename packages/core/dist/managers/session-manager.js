...
export class SessionManager {
    ...
    async parseHistoryIndex(sessionMap) {
    ...
    async enrichWithActiveSessions(sessionMap) {
        ...
        for (const file of files) {
            if (!file.endsWith('.json'))
                continue;
            try {
                const data = (await readJsonFile(join(sessionsDir, file)));
    ...
    async enrichWithProjectDirs(sessionMap) {
    ...
    async getSessionHistory(historyFile, limit = 50) {
    ...
    async getIdeInfo(pid) {
        if (!pid)
            return undefined;
        const lockFile = join(this.claudeHome, 'ide', `${pid}.lock`);
        try {
            const data = (await readJsonFile(lockFile));
            if (data.ideName) {
    ...
    async getProjectConfig(cwd) {
        if (!cwd)
            return { hasMcpJson: false, hasClaudeMd: false, hasProjectSettings: false };
        return {

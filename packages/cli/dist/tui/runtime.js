...
const MIN_COLS = 60;
const MIN_ROWS = 15;
export function canLaunchTui(caps) {
    if (!caps.isTTY)
...
export async function runTui() {
    ...
    if (!verdict.ok) {
        if (verdict.reason === 'non-tty') {
            console.error('claude-config requires a TTY. Use subcommands for scripted workflows: claude-config --help');
        }
        else {
            console.error(`claude-config TUI requires terminal ≥ ${MIN_COLS}×${MIN_ROWS}. Resize and retry.`);
        }
        return 1;
    }

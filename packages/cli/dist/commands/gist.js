...
async function ensureGhAvailable() {
    try {
        ...
        throw new Error("The `gh` CLI is required for gist sync but wasn't found on PATH.\n" +
...
function makePushCommand() {
    ...
    cmd
        ...
        if (residual.length > 0) {
            console.error(`Refusing to push — ${residual.length} value(s) still match known secret patterns after key/env scrubbing:`);
...
export function makeGistCommand() {
